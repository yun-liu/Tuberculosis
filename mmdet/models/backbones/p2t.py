...
class PyramidPoolingTransformer(BaseModule):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dims=[64, 128, 320, 512], out_indices=(0, 1, 2, 3),
                 num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0.1, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 9, 3], init_cfg=None, pretrained=None, **kwargs): #
        super(PyramidPoolingTransformer, self).__init__(init_cfg)
        self.depths = depths
