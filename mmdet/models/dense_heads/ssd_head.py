...
class SSDHead(AnchorHead):
    ...
    def __init__(self,
        ...
        if classifier is not None:
            self.classifier = nn.Sequential(
                nn.Conv2d(in_channels[1], 512, kernel_size=3, padding=1, stride=2, bias=True),
    ...
    def forward(self, feats):
        ...
        if self.classifier is not None:
            img_cls_scores = self.classifier(feats[1]).squeeze()
        for feat, reg_conv, cls_conv in zip(feats, self.reg_convs,
                                            self.cls_convs):
            cls_scores.append(cls_conv(feat))
            bbox_preds.append(reg_conv(feat))
        return cls_scores, bbox_preds, img_cls_scores
    ...
    def loss(self,
        ...
        Returns:
            dict[str, Tensor]: A dictionary of loss components.
        """
        if cfg['stage'] == "classify" or cfg['stage'] == "finetune":
            loss_img_cls = F.cross_entropy(
                img_cls_scores, gt_classes.squeeze(), weight=torch.Tensor([1., 1., 4.]).cuda(),
            )
            return dict(loss_img_cls=loss_img_cls)
