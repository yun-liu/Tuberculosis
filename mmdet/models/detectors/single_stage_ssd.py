...
class SingleStageDetectorSSD(BaseDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SingleStageDetectorSSD, self).__init__(init_cfg)
        if pretrained:
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        bbox_inputs = outs[:2] + (img_metas, self.test_cfg, rescale)
        bbox_list = self.bbox_head.get_bboxes(*bbox_inputs)
        bbox_results = [
            bbox2result(det_bboxes, det_labels, self.bbox_head.num_classes)
            for det_bboxes, det_labels in bbox_list
        ]
        return bbox_results[0], outs[-1] # VGG-based SSD detector
