...
class LoadImageFromFile:
    ...
    def __call__(self, results):
        ...
        gt_classes = np.array([0.]).astype(np.float32)
        if 'health' in filename:
            ...
            gt_classes[0] += 2
        elif 'CHNCXR' in filename:
            if osp.basename(filename)[-5] == '1':
                gt_classes[0] += 2
        elif 'MCUCXR' in filename:
            if osp.basename(filename)[-5] == '1':
                gt_classes[0] += 2
        elif 'da+db' in filename:
            if osp.basename(filename).startswith('p'):
                gt_classes[0] += 2
        results['gt_classes'] = gt_classes.astype(np.int64)
        return results
